...
@click.option('--create-plots', is_flag=True, default=True,
              help='Create summary box plots')
def analyze_impulse(data_dir, output_dir, create_plots):
    ...
    for config, weight_kg in sorted(CONFIG_WEIGHTS.items()):
        ...
        click.echo(f"   {config}: {material_name:<12} "
...
def analyze_single(file_path, material, output, show_plot, debug):
    ...
    try:
        ...
        if debug:
            click.echo("\n🔧 DEBUG INFORMATION:")
            click.echo(f"   Material: {material} ({MATERIAL_NAMES.get(material, 'Unknown')})")
            click.echo(f"   Mass breakdown: {result.get('mass_breakdown', 'N/A')}")
            click.echo(f"   Impact boundaries: {result['impact_start_idx']} to {result['impact_end_idx']}")
            click.echo(f"   Sampling rate: {result.get('sampling_rate_hz', 'Unknown'):.0f} Hz")
...
@click.option('--interactive', is_flag=True, default=True, help='Use interactive plotting (default)')
@click.option('--style', type=click.Choice(['explorer', 'simple']), default='explorer',
...
def plot_file(file_path, material, interactive, style):
    ...
    try:
        ...
        from .shared.data_processing import load_csv_file, calculate_total_force, get_time_array
        ...
        click.echo(f"📈 Data loaded: {len(force_data)} points, {time_data[-1]:.4f}s duration")
        click.echo(f"🔍 Force range: {np.min(force_data):.1f} to {np.max(force_data):.1f} N")
        ...
        if interactive and style == 'explorer':
            ...
            click.echo("💡 Controls: Mouse wheel=zoom, Click+drag=select region, Reset button=restore")
            
            from .visualization import show_force_preview_interactive
            show_force_preview_interactive(force_data, time_data, file_path.name, style=style)
        else:
...
@click.option('--style', type=click.Choice(['explorer']), default='explorer',
...
def interactive_plot(file_path, material, show_analysis, style):
    ...
    try:
        # Import visualization functions
        from .visualization import show_force_preview_interactive
        from .shared.data_processing import load_csv_file, calculate_total_force, get_time_array, extract_material_code
...
def gui(file):
    ...
    try:
        from .gui import PYQT_AVAILABLE, launch_gui
    ...
    for file_path in csv_files:
        try:
            ...
            if 'error' not in result:
                ...
                click.echo(f"{file_path.name:<18} {material:<4} {impulse:>+12.6f} N*s  {direction:<10} {peak_force:>6.0f} N")
                success_count += 1
            else:
...
def info():
    ...
    click.echo(f"   Line length: {MEASURED_LINE_LENGTH_INCHES}\" ({MEASURED_LINE_MASS_GRAMS}g measured)")
    click.echo(f"   Effective line mass: {LINE_MASS_FRACTION*100:.0f}% (literature validated)")
    ...
    click.echo("   poetry run python -m Fishing_Line_Flyback_Impact_Analysis analyze-single file.csv --show-plot")
    click.echo()
    click.echo("   # Quick check of first 5 files:")
    click.echo("   poetry run python -m Fishing_Line_Flyback_Impact_Analysis quick-check data/csv -n 5")
    click.echo()
    click.echo("   # Interactive force curve exploration:")
    click.echo("   poetry run python -m Fishing_Line_Flyback_Impact_Analysis interactive-plot data/csv/STND-21-5.csv")
    click.echo()
    
    click.echo("🗂️  LEGACY ACCESS:")
    click.echo("   Kinetic energy analysis and windowing tools are available via:")
    click.echo("   poetry run python -m Fishing_Line_Flyback_Impact_Analysis legacy --help")
