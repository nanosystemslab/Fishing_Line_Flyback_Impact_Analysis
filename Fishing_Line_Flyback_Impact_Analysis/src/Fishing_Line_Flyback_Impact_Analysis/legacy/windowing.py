...
class DataWindowingTool:
    ...
    def load_csv_file(self, csv_path: str) -> bool:
        ...
        try:
            ...
            self.force_data, self.force_columns = self.analyzer._calculate_total_force(df)
            ...
            print(f"   Force range: {np.min(self.force_data):.1f} to {np.max(self.force_data):.1f} N")
    ...
    def create_interactive_plot(self):
        ...
        self.fig.suptitle(f'Interactive Window Selection: {self.current_file.name}', fontsize=14, fontweight='bold')
    ...
    def _suggest_impact_window(self):
        ...
        if len(significant_indices) > 0:
            ...
            suggested_end = min(len(self.force_data) - 1, max_force_idx + len(significant_indices) // 4)
            ...
            if suggested_end - suggested_start < min_samples:
                suggested_end = min(len(self.force_data) - 1, suggested_start + min_samples)
            ...
            # Add peak marker
            self.ax.axvline(self.time_data[max_force_idx], color='red', linestyle='--', 
                           alpha=0.7, label=f'Peak force ({max_force:.0f} N)')
            
            self.ax.legend()
            
            print(f"💡 Suggested window: {self.time_data[suggested_start]:.4f} - {self.time_data[suggested_end]:.4f} s")
            print(f"   Duration: {(suggested_end - suggested_start) * self.analyzer.dt * 1000:.1f} ms")
            print(f"   Peak force: {max_force:.0f} N at {self.time_data[max_force_idx]:.4f} s")
    ...
    def on_window_select(self, start_time: float, end_time: float):
        ...
        self.ax_zoom.set_title(f'Selected Window: {duration_ms:.1f} ms, Peak: {max_force_in_window:.0f} N')
        self.ax_zoom.grid(True, alpha=0.3)
        
        # Add peak marker in zoom view
        peak_idx_in_window = np.argmax(np.abs(window_force))
        self.ax_zoom.axvline(window_time[peak_idx_in_window], color='red', linestyle='--', alpha=0.7)
        
        self.fig.canvas.draw()
        
        print(f"📏 Selected window: {start_time:.4f} - {end_time:.4f} s ({duration_ms:.1f} ms)")
    ...
    def analyze_selected_window(self, event):
        ...
        window_time = self.time_data[self.window_start_idx:self.window_end_idx+1] - self.time_data[self.window_start_idx]
        
        print(f"\n🔬 ANALYZING SELECTED WINDOW:")
        print(f"   Time range: {self.time_data[self.window_start_idx]:.4f} - {self.time_data[self.window_end_idx]:.4f} s")
        print(f"   Duration: {(self.window_end_idx - self.window_start_idx) * self.analyzer.dt * 1000:.1f} ms")
        ...
        for material in materials_to_test:
            try:
                ...
                test_analyzer = ImpactAnalyzer(material_code=material, include_line_mass=True)
                
                # Analyze windowed data
                result = test_analyzer.calculate_corrected_energy(window_force, window_time)
                ...
                print(f"{material:<8} {mass_g:<4.0f}g  {velocity:<6.0f} m/s {energy:<6.1f} J  {status}")
    ...
    def save_window_selection(self, event):
        ...
        window_info = {
            ...
            'duration_ms': float((self.window_end_idx - self.window_start_idx) * self.analyzer.dt * 1000),
    ...
    def create_windowed_csv(self, window_info: Dict):
        ...
        try:
            ...
            # Save windowed CSV
            output_csv = self.current_file.parent / f"{self.current_file.stem}_windowed.csv"
...
def batch_window_files(data_dir: str, pattern: str = "*.csv", 
                      auto_suggest: bool = True) -> List[Dict]:
    """
        ...
        try:
            if tool.load_csv_file(file_path):
                ...
                if len(significant_indices) > 0:
                    ...
                    end_idx = min(len(tool.force_data) - 1, 
                    ...
                    print(f"✅ {file_path.name}: {duration_ms:.1f}ms window, peak {max_force:.0f}N")
                else:
...
def review_window_suggestions(data_dir: str, suggestions_file: str = None):
    ...
    print("-" * 60)
    print(f"{'Filename':<15} {'Duration':<8} {'Peak Force':<10} {'Est Velocity':<12} {'Status'}")
    print("-" * 60)
    ...
    for suggestion in test_suggestions:
        ...
        try:
            ...
            print(f"{filename:<15} {suggestion['duration_ms']:<6.1f}ms {suggestion['peak_force']:<8.0f}N {velocity:<8.0f} m/s {status}")
    ...
    if priority_files:
        print(f"   🔴 HIGH PRIORITY - Manual review needed ({len(priority_files)} files):")
        for filename in priority_files[:10]:  # Show first 10
            print(f"      python -m Fishing_Line_Flyback_Impact_Analysis window-tool data/csv/{filename}")
    ...
    if max_windows and len([s for s in max_windows if s['filename'] not in priority_files]) > 0:
        medium_priority = [s['filename'] for s in max_windows if s['filename'] not in priority_files]
        print(f"   🟡 MEDIUM PRIORITY - Check for rebound inclusion ({len(medium_priority)} files)")
    ...
    print(f"   python -m Fishing_Line_Flyback_Impact_Analysis analyze-windowed data/csv")
