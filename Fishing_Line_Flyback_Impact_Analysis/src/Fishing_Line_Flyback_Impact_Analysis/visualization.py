...
RMS Force: {np.sqrt(np.mean(force_data**2)):.1f} N'''
    ...
    ax2.axvline(peak_time_ms, color='orange', linestyle='--', alpha=0.7, label=f'Peak at {peak_time_ms:.1f} ms')
    ax2.scatter(peak_time_ms, force_data[peak_idx], color='red', s=100, zorder=5, label=f'Peak: {force_data[peak_idx]:.1f} N')
    ...
    if show_analysis_preview:
        try:
            ...
            for ax in [ax1, ax2]:
                ax.axvline(time_ms[impact_start], color='green', linestyle=':', alpha=0.8, label='Impact Start')
                ax.axvline(time_ms[impact_end], color='green', linestyle=':', alpha=0.8, label='Impact End')
                ax.legend()
            ...
            ax2.text(0.98, 0.02, analysis_text, transform=ax2.transAxes,
...
def plot_single_file_analysis(csv_path: Union[str, Path], 
    ...
    try:
        ...
        fig = create_single_file_analysis_plot(force_data, time_data, result, csv_path.name)
...
def create_single_file_analysis_plot(force_data: np.ndarray, time_data: np.ndarray,
                                   analysis_result: Dict, filename: str) -> plt.Figure:
    """
    ...
    ax1.axvline(time_ms[impact_start], color='red', linestyle='--', linewidth=2, label='Impact Start')
    ax1.axvline(time_ms[impact_end], color='red', linestyle='--', linewidth=2, label='Impact End')
    
    # Highlight impact region
    impact_mask = np.zeros_like(force_data, dtype=bool)
    impact_mask[impact_start:impact_end+1] = True
    ax1.fill_between(time_ms, force_data, where=impact_mask, alpha=0.3, color='green', label='Impact Region')
    ...
    if impact_end > impact_start:
        ...
        ax2.axvline(time_ms[impact_start], color='green', linestyle='--', linewidth=2, label='Start')
        ax2.axvline(time_ms[impact_end], color='green', linestyle='--', linewidth=2, label='End')
        ...
        ax2.fill_between(impact_time_zoom, impact_force_zoom, alpha=0.4, color='green', label='Integration Area')
        
        # Mark peak
        peak_idx = impact_start + np.argmax(np.abs(force_data[impact_start:impact_end+1]))
        ax2.scatter(time_ms[peak_idx], force_data[peak_idx], color='red', s=100, zorder=5, label=f'Peak: {force_data[peak_idx]:.0f} N')
    ...
    ax3.plot(time_ms, cumulative_impulse, 'purple', linewidth=2, label='Cumulative Impulse')
    ax3.axhline(total_impulse, color='orange', linestyle=':', linewidth=2, label=f'Total: {total_impulse:+.6f} N⋅s')
...
def create_impulse_material_comparison(results: List[Dict], output_dir: Union[str, Path]) -> None:
    """
...
def create_impulse_summary_statistics(results: List[Dict], output_dir: Union[str, Path]) -> str:
    """
...
def create_interactive_force_explorer(force_data: np.ndarray, time_data: np.ndarray, 
                                    filename: str, show_analysis_preview: bool = False) -> None:
    """
    ...
    fig.suptitle(f'Interactive Force Explorer: {filename}', fontsize=14, fontweight='bold')
    
    # Main plot with full data
    line_main, = ax_main.plot(time_ms, force_data, 'b-', linewidth=1, alpha=0.8, label='Force')
    ...
    if show_analysis_preview:
        try:
            ...
            start_line = ax_main.axvline(time_ms[impact_start], color='green', linestyle='--', 
                                       linewidth=2, alpha=0.7, label='Impact Start')
            end_line = ax_main.axvline(time_ms[impact_end], color='red', linestyle='--', 
                                     linewidth=2, alpha=0.7, label='Impact End')
            boundary_lines = [start_line, end_line]
            
            # Highlight impact region
            impact_region = patches.Rectangle((time_ms[impact_start], ax_main.get_ylim()[0]), 
                                            time_ms[impact_end] - time_ms[impact_start],
                                            ax_main.get_ylim()[1] - ax_main.get_ylim()[0],
                                            alpha=0.2, facecolor='yellow', label='Impact Region')
...
• Use Reset button to restore view"""
    ...
    def onselect(xmin, xmax):
        ...
        if np.any(time_mask):
            ...
            ax_zoom.set_title(f'Zoomed View: {xmin:.1f} - {xmax:.1f} ms ({xmax-xmin:.1f} ms duration)')
...
def create_dual_view_explorer(force_data: np.ndarray, time_data: np.ndarray, 
                             filename: str) -> None:
    """
    ...
    # Selection rectangle
    selection_rect = patches.Rectangle((0, 0), 0, 0, linewidth=2, 
...
Right-click to reset selection"""
    ...
    def on_press(event):
        ...
        if event.inaxes == ax_overview and event.button == 1:  # Left click
            ...
            selection_rect.set_height(ax_overview.get_ylim()[1] - ax_overview.get_ylim()[0])
        ...
        if selecting and event.inaxes == ax_overview and event.button == 1:
            ...
            if start_x is not None and end_x is not None:
                ...
                if np.any(time_mask):
                    ...
                    ax_detail.set_ylim(np.min(detail_force) * 1.1, np.max(detail_force) * 1.1)
...
def show_force_preview_interactive(force_data: np.ndarray, time_data: np.ndarray, 
                                 filename: str, show_analysis_preview: bool = False, 
                                 style: str = "explorer"):
    """
    ...
    if style != "explorer":
        ...
        return show_force_preview(force_data, time_data, filename, show_analysis_preview)
    ...
    gs = fig.add_gridspec(4, 3, height_ratios=[3, 2, 2, 1], width_ratios=[4, 1, 1], 
                         hspace=0.3, wspace=0.3)
    
    # Main plot (top, spans all columns)
    ax_main = fig.add_subplot(gs[0, :])
    line_main, = ax_main.plot(time_ms, force_data, 'b-', linewidth=0.5, alpha=0.8, label='Force')
    ax_main.set_xlabel('Time (ms)', fontsize=12)
    ax_main.set_ylabel('Force (N)', fontsize=12)
    ax_main.set_title(f'Interactive Force Explorer: {filename}', fontsize=14, fontweight='bold')
    ax_main.grid(True, alpha=0.3)
    ax_main.legend()
    ...
    ax_controls.text(0.1, 0.9, 'CONTROLS', fontweight='bold', fontsize=12, transform=ax_controls.transAxes)
    ax_controls.text(0.1, 0.7, '• Mouse wheel:\n  Zoom in/out', fontsize=9, transform=ax_controls.transAxes)
    ax_controls.text(0.1, 0.5, '• Click + drag:\n  Select region', fontsize=9, transform=ax_controls.transAxes)
    ax_controls.text(0.1, 0.3, '• Reset button:\n  Restore view', fontsize=9, transform=ax_controls.transAxes)
    ax_controls.text(0.1, 0.1, '• Close window:\n  Exit', fontsize=9, transform=ax_controls.transAxes)
...
Peak: {np.max(np.abs(force_data)):.0f} N"""
    ...
    ax_info.text(0.1, 0.9, data_info, fontsize=9, fontfamily='monospace',
    ...
    if show_analysis_preview:
        try:
            ...
            # Add boundary lines to main plot
            ax_main.axvline(time_ms[impact_start], color='green', linestyle='--', 
                           linewidth=2, alpha=0.7, label='Impact Start')
            ax_main.axvline(time_ms[impact_end], color='red', linestyle='--', 
                           linewidth=2, alpha=0.7, label='Impact End')
            
            # Highlight impact region
            impact_region = patches.Rectangle((time_ms[impact_start], original_ylim[0]), 
                                            time_ms[impact_end] - time_ms[impact_start],
...
Zoom Level: {(time_ms[-1] - time_ms[0]) / duration_view:.1f}x"""
    ...
    peak_button = Button(peak_button_axes, 'Zoom to Peak', color='lightgreen', hovercolor='green')
