...
from .analysis import MEASURED_LINE_LENGTH_INCHES
from .analysis import MEASURED_LINE_MASS_GRAMS
...
def analyze_all(data_dir, output_dir, show_progress):
    ...
    for config, weight_kg in sorted(CONFIG_WEIGHTS.items()):
        ...
        click.echo(
            f"   {config:4s}: {weight_kg*1000:2.0f}g + {line_mass_effective*1000:.0f}g = {total_mass*1000:.0f}g total"
        )
    click.echo()
    ...
    click.echo(
        f"   Effective mass: {LINE_MASS_FRACTION*100:.0f}% (literature: Cartmell & McKenzie 2008)"
    )
    ...
    try:
        ...
        if results:
            ...
            click.echo(
                f"🎯 Target velocity range: {target_count}/{len(valid_results)} ({target_count/len(valid_results)*100:.1f}%)"
            )
...
def analyze_single(file_path, material, output, debug, legacy_mass):
    ...
    try:
        if legacy_mass is not None:
            ...
            click.echo(
                f"   Line (effective): {line_mass_effective*1000:.0f}g ({LINE_MASS_FRACTION*100:.0f}%)"
            )
        ...
        click.echo(
            f"   Impact Duration: {result.get('impact_duration', 0)*1000:.1f} ms"
        )
        ...
        if debug:
            ...
            click.echo(
                f"   Overestimation factor: {result.get('overestimation_factor', 1):.2f}x"
            )
...
def analyze_impulse_single(file_path, material, output, debug):
    ...
    try:
        ...
        if "equivalent_velocity" in result and not np.isnan(
            result["equivalent_velocity"]
        ):
...
def compare_methods(data_dir, output_dir):
    ...
    try:
        ...
        ke_results = run_comprehensive_analysis(
            data_dir, output_path / "kinetic_energy"
        )
    ...
    for file_path in csv_files:
        try:
            ...
            if "error" not in result:
                ...
                click.echo(
                    f"✅ {file_path.name:<15} | {material:<4} | {impulse:>+10.6f} N⋅s {direction} | "
...
def show_config():
    ...
    click.echo(
        f"   Linear density: {MEASURED_LINE_MASS_GRAMS/(MEASURED_LINE_LENGTH_INCHES*0.0254):.1f} g/m"
    )
    ...
    for config, weight_kg in sorted(CONFIG_WEIGHTS.items()):
        ...
        total_mass = weight_kg + line_mass_effective
        click.echo(
            f"   {config:4s}: {weight_kg*1000:2.0f}g hardware + {line_mass_effective*1000:.0f}g line = {total_mass*1000:.0f}g total"
        )
    click.echo()
...
def info():
    ...
    click.echo(
        "poetry run python -m Fishing_Line_Flyback_Impact_Analysis analyze-impulse-single data/csv/STND-21-5.csv --debug"
    )
    click.echo()
    ...
    click.echo(
        "poetry run python -m Fishing_Line_Flyback_Impact_Analysis analyze-all data/csv"
    )
    click.echo()
...
def quick_batch_check(data_dir, count):
    ...
    for file_path in csv_files:
        try:
            ...
            if "error" not in result:
                ...
                click.echo(
                    f"{status} {file_path.name:<15} | {material:<4} | {velocity:3.0f} m/s | {energy:6.1f} J"
                )
                success_count += 1
            else:
                click.echo(f"❌ {file_path.name:<15} | ERROR: {result['error']}")
    ...
    if success_count > 0:
        ...
        click.echo(f"📊 Summary: {success_count}/{len(csv_files)} successful")
        click.echo(
            f"🎯 Target range: {target_count}/{success_count} ({target_count/success_count*100:.1f}%)"
        )
...
def analyze_impulse_single(file_path, material, output, debug, show_plot):
    ...
    try:
        ...
        if show_plot:
            ...
            click.echo(
                f"   Duration should typically be 2-50 ms for fishing line impacts"
            )
...
def validate_boundaries(file_path, material):
    ...
    try:
        ...
        if 2 <= duration <= 50:
            ...
            click.echo(
                f"   ⚠️  Duration very short ({duration:.1f} ms) - may miss impact data"
            )
        elif duration > 100:
            click.echo(
                f"   ❌ Duration very long ({duration:.1f} ms) - likely includes rebound/noise"
            )
        else:
            click.echo(
                f"   🟡 Duration borderline ({duration:.1f} ms) - review plot carefully"
            )
