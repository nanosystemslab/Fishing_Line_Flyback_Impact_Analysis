...
class ImpactAnalyzer:
    ...
    def _load_csv_file(self, filepath: str) -> pd.DataFrame:
        ...
        sensor_cols = [col for col in df.columns if col.startswith('S') and col != 'SUM']
        if sensor_cols:
    ...
    def calculate_impact_properties(self, df: pd.DataFrame, 
                                  param_y: str = "SUM", 
                                  param_x: str = "time") -> Dict[str, float]:
    ...
    def load_results_file(self, filepath: str) -> pd.DataFrame:
        ...
        df.loc[:, df.columns != "fname"] = df.loc[:, df.columns != "fname"].astype(float)
        ...
        bad_files = [
            "STND-21-1", "STND-21-3", "DF-21-6", "DF-21-7", 
            "DS-21-1", "DS-21-8", "SL-21-8"
        ]
    ...
    def calculate_summary_stats(self, results_df: pd.DataFrame) -> pd.DataFrame:
        ...
        results = pd.DataFrame(columns=[
            'test_type', 'mean_J', 'std_J', 'var_J', 
            'mean_F', 'std_F', 'var_F', '%diff_J', '%diff_F'
        ])
        ...
        for test_type in test_types:
            ...
            if test_type == "STND":
                ...
                diff_J = (val_mean["J"] / stnd_mean["J"]) - 1 if stnd_mean is not None else 0
                diff_F = (val_mean["F"] / stnd_mean["F"]) - 1 if stnd_mean is not None else 0
            
            results.loc[len(results)] = [
                test_type, val_mean["J"], val_std["J"], val_var["J"],
                val_mean["F"], val_std["F"], val_var["F"], diff_J, diff_F
            ]
        
        return results

    def generate_summary_table(self, results_df: pd.DataFrame, output_dir: str = None) -> Dict[str, Any]:
        ...
        for config in ['STND', 'DF', 'DS', 'SL', 'BR']:
            ...
            energy_mj = ((config_data['J'] * 4.44822)**2) / (2 * mass_kg) / 1e6  # to MJ
        ...
        if stnd_stats:
            for config in config_stats:
                if config != 'STND':
                    config_stats[config]['impulse_pct'] = ((config_stats[config]['impulse_mean'] / stnd_stats['impulse_mean']) - 1) * 100
                    config_stats[config]['energy_pct'] = ((config_stats[config]['energy_mean'] / stnd_stats['energy_mean']) - 1) * 100
                else:
        ...
        if output_dir:
            ...
            with open(summary_file, 'w') as f:
                ...
                f.write("\n\nNOTES:\n")
                f.write("- Impact Energy calculated assuming inelastic collision: E = impulse²/(2×mass)\n")
                f.write("- Percentages calculated relative to Standard (SD) configuration\n")
    ...
    def _print_summary_table(self, config_stats: Dict[str, Any]) -> None:
        ...
        # Header
        print(f"{'Configuration':<16} {'Runs':<5} {'Impulse':<12} {'STD':<8} {'% vs':<8} "
              f"{'Max Force':<12} {'STD':<8} {'Impact Energy':<14} {'% vs':<8}")
        print(f"{'':16} {'':5} {'[kN·s]':<12} {'':8} {'STND':<8} "
        ...
        for config in ['STND', 'DF', 'DS', 'SL', 'BR']:
            if config in config_stats:
                ...
                print(f"{stats['name']:<16} {stats['n_runs']:<5} "
        ...
        if 'STND' in config_stats and 'BR' in config_stats:
            ...
            print(f"• Breakaway reduces impact energy by {reduction:.0f}% vs Standard")
        ...
        if 'STND' in config_stats:
            ...
            if best_alternative:
                print(f"• {config_stats[best_alternative]['name']} shows best compromise "
    ...
    def _write_table_to_file(self, f, config_stats: Dict[str, Any]) -> None:
        ...
        f.write(f"{'Configuration':<16} {'Runs':<5} {'Impulse':<12} {'STD':<8} {'% vs':<8} "
                f"{'Max Force':<12} {'STD':<8} {'Impact Energy':<14} {'% vs':<8}\n")
        f.write(f"{'':16} {'':5} {'[kN·s]':<12} {'':8} {'STND':<8} "
                f"{'[kN]':<12} {'':8} {'[MJ]':<14} {'STND':<8}\n")
        f.write("-" * 100 + "\n")
        ...
        for config in ['STND', 'DF', 'DS', 'SL', 'BR']:
            if config in config_stats:
                stats = config_stats[config]
                f.write(f"{stats['name']:<16} {stats['n_runs']:<5} "
                        f"{stats['impulse_mean']:<12.2f} {stats['impulse_std']:<8.2f} "
                        f"{stats['impulse_pct']:<+8.0f}% "
                        f"{stats['force_mean']:<12.2f} {stats['force_std']:<8.2f} "
                        f"{stats['energy_mean']:<14.0f} {stats['energy_pct']:<+8.0f}%\n")
    ...
    def generate_latex_table(self, results_df: pd.DataFrame, output_dir: str) -> None:
        ...
        with open(latex_file, 'w') as f:
            ...
            f.write("\\hline\n")
            f.write("Configuration & \\#Runs & Impulse & STD & \\% vs & Max Force & STD & Avg. Impact & \\% vs \\\\\n")
            f.write("              &        & [kN·s]  &     & STND  & [kN]      &     & Energy [MJ] & STND \\\\\n")
            f.write("\\hline\n")
            ...
            for config in ['STND', 'DF', 'DS', 'SL', 'BR']:
                if config in config_stats:
                    stats = config_stats[config]
                    f.write(f"{stats['name']} & {stats['n_runs']} & "
                           f"{stats['impulse_mean']:.2f} & {stats['impulse_std']:.2f} & "
                           f"{stats['impulse_pct']:+.0f}\\% & "
                           f"{stats['force_mean']:.2f} & {stats['force_std']:.2f} & "
                           f"{stats['energy_mean']:.0f} & {stats['energy_pct']:+.0f}\\% \\\\\n")
            
            f.write("\\hline\n")
            f.write("\\end{tabular}\n")
            f.write("\\caption{Flyback impact performance comparison across gear configurations. Impact Energy calculated assuming inelastic collision.}\n")
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"{'Configuration':<15} {'Runs':<5} {'Impulse':<10} {'±STD':<8} {'%STND':<8} "
                   f"{'Force':<10} {'±STD':<8} {'Energy':<10} {'%STND':<8}\n")
            f.write(f"{'':15} {'':5} {'(kN·s)':<10} {'':8} {'':8} "
                   f"{'(kN)':<10} {'':8} {'(MJ)':<10} {'':8}\n")
            f.write("-" * 90 + "\n")
            ...
            for config in ['STND', 'DF', 'DS', 'SL', 'BR']:
                if config in config_stats:
                    stats = config_stats[config]
                    f.write(f"{stats['name']:<15} {stats['n_runs']:<5} "
    ...
    def generate_summary_report(self, results: pd.DataFrame, output_dir: str) -> None:
        ...
        with open(report_path, 'w') as f:
            ...
            for _, row in results.iterrows():
                ...
                f.write(f"  Mean Impulse: {row['mean_J']:.3f} ± {row['std_J']:.3f} Ns\n")
