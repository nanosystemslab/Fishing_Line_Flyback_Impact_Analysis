...
def calculate_line_mass_from_measurement(
    measured_length_inches: float = MEASURED_LINE_LENGTH_INCHES,
    measured_mass_grams: float = MEASURED_LINE_MASS_GRAMS,
    active_length_meters: float = ACTIVE_LINE_LENGTH_METERS,
) -> Dict:
    """
...
class ImpactAnalyzer:
    ...
    def print_mass_info(self):
        ...
        if "legacy_mode" in self.mass_breakdown:
            ...
            if self.include_line_mass:
                ...
                print(
                    f"   Line (effective): {mb['line_mass_effective_kg']*1000:.1f}g ({mb['line_mass_fraction']*100:.0f}%)"
                )
                print(f"   Total system: {mb['total_system_mass_kg']*1000:.1f}g")
                print(
                    f'   Line measurement: {MEASURED_LINE_LENGTH_INCHES}" = {MEASURED_LINE_MASS_GRAMS}g'
                )
                print(f"   Active length: {ACTIVE_LINE_LENGTH_METERS}m")
            else:
                print(
                    f"   Total system: {mb['total_system_mass_kg']*1000:.1f}g (hardware only)"
                )
    ...
    def find_peak_impact_region(
        self, force: np.ndarray, debug: bool = False
    ) -> Tuple[int, int]:
        """
        ...
        for threshold_pct in [90, 80, 70, 60, 50]:
            ...
            if len(peak_indices) > 0:
                ...
                if debug:
                    print(
                        f"      {threshold_pct}% threshold ({threshold:.0f} N): "
                ...
                if 100 <= v_peak <= 500:
                    if debug:
                        print(
                            f"      ✅ Found reasonable velocity with {threshold_pct}% threshold"
                        )
    ...
    def calculate_corrected_energy(
        self, force: np.ndarray, time: Optional[np.ndarray] = None, debug: bool = False
    ) -> Dict:
        """
        ...
        if debug:
            ...
            print(
                f"      Start: index {impact_start}, time {impact_start * self.dt:.6f} s"
            )
            print(f"      End: index {impact_end}, time {impact_end * self.dt:.6f} s")
            print(f"      Duration: {impact_duration * 1000:.2f} ms")
            print(
                f"      Force range: {np.min(force_peak):.1f} to {np.max(force_peak):.1f} N"
            )
        ...
        if SCIPY_AVAILABLE and len(force_peak) > 4:
            ...
            J_peak = (
                np.trapz(force_peak, dx=self.dt)
                if len(force_peak) > 1
                else np.sum(force_peak) * self.dt
            )
            integration_method = "Trapezoidal (NumPy)"

        # Calculate total impulse for comparison (legacy method)
        J_total = (
            np.trapz(force, dx=self.dt) if len(force) > 1 else np.sum(force) * self.dt
        )
        ...
        return {
            ...
            "overestimation_factor": (
                kinetic_energy_overestimated / kinetic_energy_corrected
                if kinetic_energy_corrected > 0
                else 1.0
            ),
    ...
    def batch_analyze_directory(
        self, data_dir: Union[str, Path], file_pattern: str = "*.csv"
    ) -> List[Dict]:
        """
        ...
        if self.include_line_mass:
            print(
                f"   Using {self.line_mass_fraction*100:.0f}% line mass ({self.mass*1000:.1f}g total)"
            )
        else:
        ...
        for file_path in files:
            ...
            if "error" not in result:
                ...
                status = (
                    "✅"
                    if 150 <= velocity <= 400
                    else ("🟡" if 100 <= velocity <= 500 else "❌")
                )
                print(
                    f"  {status} {file_path.name}: {velocity:.0f} m/s, {result['kinetic_energy']:.1f} J"
                )
            else:
...
def analyze_single_file_with_config(
...
def run_comprehensive_analysis(
    ...
    output_dir: Union[str, Path] = "comprehensive_analysis",
) -> List[Dict]:
    """
    ...
    print(
        f"📊 Using enhanced peak-focused analysis with configuration-specific weights"
    )
    print(
        f'⚖️  Line measurement: {MEASURED_LINE_LENGTH_INCHES}" = {MEASURED_LINE_MASS_GRAMS}g → 38.8g total for 10m'
    )
    ...
    for config, weight_kg in sorted(CONFIG_WEIGHTS.items()):
        ...
        print(
            f"   {config:4s}: {weight_kg*1000:2.0f}g + {effective_line_mass_kg*1000:.0f}g = {total_kg*1000:.0f}g total"
        )
    print()
    ...
    print("-" * 80)
    print(
        f"{'Status':<2} {'Filename':<15} {'Mat':<4} {'Weight':<6} {'Total':<6} {'Velocity':<8} {'Energy':<10}"
    )
    print("-" * 80)
    ...
    for filename in files:
        ...
        try:
            ...
            if "error" not in result:
                ...
                print(
                    f"{status} {filename:<15} | {material:<4} | {config_weight_g:3.0f}g | {total_mass_g:3.0f}g | {velocity:3.0f} m/s | {energy:6.1f} J"
                )
                results.append(result)
...
def create_summary_report(results: List[Dict], output_dir: Path):
    ...
    print(
        f"   Target range (150-400 m/s): {target_count}/{len(velocities)} ({target_count/len(velocities)*100:.1f}%)"
    )
    print(
        f"   Reasonable range (100-500 m/s): {reasonable_count}/{len(velocities)} ({reasonable_count/len(velocities)*100:.1f}%)"
    )
    ...
    print(
        f"   {'Material':<8} {'Count':<5} {'Weight':<7} {'Target%':<7} {'Velocity':<12} {'Energy':<10} {'Consistency'}"
    )
    ...
    for mp in material_performance:
        consistency = (
            "High"
            if mp["velocity_std"] < 50
            else ("Med" if mp["velocity_std"] < 100 else "Low")
        )
        total_mass = mp["config_weight"] + (LINE_MASS_FRACTION * 38.8)
        print(
            f"   {mp['material']:<8} {mp['count']:<5} {mp['config_weight']:<4.0f}g   {mp['target_percent']:<6.1f}% "
    ...
    config_vs_velocity = [
        (mp["config_weight"], mp["mean_velocity"]) for mp in material_performance
    ]
    ...
    for weight, velocity in config_vs_velocity:
        material_name = [
            k for k, v in CONFIG_WEIGHTS.items() if abs(v * 1000 - weight) < 1
        ][0]
    ...
    with open(report_path, "w") as f:
        ...
        for config, weight_kg in sorted(CONFIG_WEIGHTS.items()):
            total_kg = weight_kg + effective_line_mass_kg
            f.write(
                f"   {config}: {weight_kg*1000:.0f}g + {effective_line_mass_kg*1000:.0f}g = {total_kg*1000:.0f}g\n"
            )
        f.write("\n")
        ...
        for i, mp in enumerate(material_performance, 1):
            f.write(
                f"{i}. {mp['material']}: {mp['target_percent']:.1f}% target achievement\n"
            )
            f.write(
                f"   Velocity: {mp['mean_velocity']:.0f} ± {mp['velocity_std']:.0f} m/s\n"
            )
...
def analyze_single_file(
    file_path: Union[str, Path], mass: float = None, **kwargs
) -> Dict:
    """
