...
# Configure matplotlib to use DejaVu Sans (which supports Unicode)
...
from .analysis import CONFIG_WEIGHTS, get_system_mass, LINE_MASS_FRACTION
...
class ImpulseAnalyzer:
    ...
    def plot_impulse_boundaries(self, force: np.ndarray, time: np.ndarray, 
        ...
        fig.suptitle(f'Impulse Integration Boundary Validation: {filename}', 
                     fontsize=max(10, min(16, fig_width)), fontweight='bold', fontfamily='DejaVu Sans')
        ...
        if impact_start < len(time) and impact_end < len(time):
            ...
            ax1.fill_between(integration_time, integration_force, alpha=0.3, color='green', 
                            label=f'Integration Region ({(impact_end - impact_start) * self.dt * 1000:.1f} ms)')
            
            # Mark boundaries
            ax1.axvline(time[impact_start], color='red', linestyle='--', linewidth=2, 
                       label=f'Start: {time[impact_start]:.4f} s')
            ax1.axvline(time[impact_end], color='red', linestyle='--', linewidth=2, 
                       label=f'End: {time[impact_end]:.4f} s')
        
        # Mark peak
        peak_idx = np.argmax(np.abs(force))
        ax1.axvline(time[peak_idx], color='orange', linestyle=':', linewidth=2, 
        ...
        ax1.set_title('Full Force Trace with Integration Boundaries', fontfamily='DejaVu Sans')
        ...
        if impact_start < len(time) and impact_end < len(time):
            ...
            ax2.fill_between(integration_time, integration_force, alpha=0.3, color='green', 
...
Peak: {np.max(np.abs(integration_force)):.1f} N
        ...
        if impact_start < len(time) and impact_end < len(time):
            ...
            print(f"   Percentage of total data: {(impact_end - impact_start + 1) / len(force) * 100:.1f}%")
    ...
    def find_impact_boundaries(self, force: np.ndarray, debug: bool = False, 
                              show_plot: bool = False, filename: str = "unknown") -> Tuple[int, int]:
        """
        ...
        if debug:
            ...
            print(f"      Peak: {peak_force:.1f} N at index {peak_idx} ({peak_idx * self.dt:.4f} s)")
        
        # STEP 2: Calculate true baseline from quiet regions
        # Use first and last 5% of data to get baseline
        baseline_samples = max(100, len(force) // 20)  # At least 100 samples, or 5% of data
        ...
        significance_threshold = max(3 * noise_level, peak_force * 0.01)  # At least 1% of peak
        ...
        impact_start = 0
        for i in range(max(0, peak_idx - 5000), peak_idx):  # Look back further if needed
        ...
        for i in range(search_start, len(force)):
            ...
            if force_deviation <= significance_threshold:
                ...
                samples_to_check = min(100, len(force) - i)  # Check next 100 samples or to end
        ...
        if duration_ms > 1000:
            if debug:
                print(f"   ⚠️  Duration very long ({duration_ms:.1f} ms), may include multiple events")
        ...
        if debug:
            ...
            print(f"      Start: index {impact_start}, time {impact_start * self.dt:.4f} s")
            print(f"      End: index {impact_end}, time {impact_end * self.dt:.4f} s")
            print(f"      Duration: {duration_ms:.1f} ms")
            print(f"      Force at start: {force[impact_start]:.1f} N (deviation: {abs(force[impact_start] - baseline):.1f} N)")
            print(f"      Force at end: {force[impact_end]:.1f} N (deviation: {abs(force[impact_end] - baseline):.1f} N)")
            print(f"      Peak force: {force[peak_idx]:.1f} N")
        
        if show_plot:
            time_array = np.arange(len(force)) * self.dt
            self.plot_impulse_boundaries_with_smoothing(force, force_smooth, time_array, 
                                                       impact_start, impact_end, filename)
        
        return impact_start, impact_end
    
    def plot_impulse_boundaries_with_smoothing(self, force_orig: np.ndarray, force_smooth: np.ndarray, 
                                              time: np.ndarray, impact_start: int, impact_end: int, 
        ...
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(fig_width, fig_height))
        fig.suptitle(f'Impulse Integration with Signal Smoothing: {filename}', 
                     fontsize=max(10, min(16, fig_width)), fontweight='bold', fontfamily='DejaVu Sans')
        
        # Top plot: Full trace with both original and smoothed
        ax1.plot(time, force_orig, 'b-', linewidth=0.5, alpha=0.6, label='Original Force')
        ax1.plot(time, force_smooth, 'r-', linewidth=1.5, alpha=0.8, label='Smoothed Force')
        
        # Highlight integration region
        if impact_start < len(time) and impact_end < len(time):
            integration_time = time[impact_start:impact_end + 1]
            integration_force = force_orig[impact_start:impact_end + 1]  # Use original for integration
            ax1.fill_between(integration_time, integration_force, alpha=0.3, color='green', 
                            label=f'Integration Region ({(impact_end - impact_start) * self.dt * 1000:.1f} ms)')
        ...
        ax1.set_xlabel('Time (s)', fontfamily='DejaVu Sans')
        ax1.set_ylabel('Force (N)', fontfamily='DejaVu Sans')
        ax1.set_title('Full Force Trace (Original + Smoothed) with Integration Boundaries', fontfamily='DejaVu Sans')
        ax1.legend(fontsize=max(8, min(10, fig_width*0.8)))
        ax1.grid(True, alpha=0.3)
        ...
        if impact_start < len(time) and impact_end < len(time):
            ...
            ax2.plot(zoom_time, zoom_force_orig, 'b-', linewidth=1, alpha=0.7, label='Original Force')
            ax2.plot(zoom_time, zoom_force_smooth, 'r-', linewidth=2, alpha=0.8, label='Smoothed Force')
            ax2.fill_between(integration_time, integration_force, alpha=0.3, color='green', 
                            label='Integration Region')
            ax2.axvline(time[impact_start], color='red', linestyle='--', linewidth=2)
            ax2.axvline(time[impact_end], color='red', linestyle='--', linewidth=2)
            ...
            ax2.set_xlabel('Time (s)', fontfamily='DejaVu Sans')
            ax2.set_ylabel('Force (N)', fontfamily='DejaVu Sans')
            ax2.set_title('Zoomed View: Full Impact Spike (Start to Return to Baseline)', fontfamily='DejaVu Sans')
            ax2.legend(fontsize=max(8, min(10, fig_width*0.8)))
            ax2.grid(True, alpha=0.3)
...
End Force: {force_orig[impact_end]:.1f} N
    ...
    def calculate_impulse_metrics(self, force: np.ndarray, time: np.ndarray,
                                debug: bool = False, show_plot: bool = False, 
        ...
        impact_start, impact_end = self.find_impact_boundaries(force, debug, show_plot, filename)
    ...
    def analyze_csv_file(self, csv_path: Union[str, Path], show_plot: bool = False) -> Dict:
        ...
        try:
            ...
            results = self.calculate_impulse_metrics(force_n, time, show_plot=show_plot, 
...
def run_impulse_analysis(data_dir: Union[str, Path] = "data/csv",
    ...
    for config, weight_kg in sorted(CONFIG_WEIGHTS.items()):
        ...
        print(f"   {config:4s}: {weight_kg*1000:2.0f}g + {line_mass_kg*1000:.0f}g = {total_kg*1000:.0f}g total")
    print()
    ...
    print("-" * 80)
    print(f"{'Status':<2} {'Filename':<20} {'Mat':<4} {'Total Impulse':<15} {'Peak Force':<12} {'Duration'}")
    print("-" * 80)
    ...
    for file_path in csv_files:
        try:
            ...
            if 'error' not in result:
                ...
                print(f"✅ {file_path.name:<20} {material:<4} {impulse:>+12.6f} N⋅s "
...
def create_impulse_visualizations(results: List[Dict], output_dir: Path):
    ...
    if 'material_type' in df.columns:
        ...
        existing_materials = [m for m in material_code_order if m in df['material_type'].values]
        print(existing_materials)
        if existing_materials:
            df['material_type'] = pd.Categorical(df['material_type'], 
                                               categories=existing_materials, ordered=True)
    ...
    sns.boxplot(data=df, x='material_type', y='total_impulse', ax=ax, order=existing_materials)
    ax.set_title('Total Impulse = ∫ F(t) dt\n(Signed Momentum Transfer)', 
    ...
    fig, ax = plt.subplots(1, 1, figsize=(10, 8))
    sns.boxplot(data=df, x='material_type', y='total_abs_impulse', ax=ax, order=existing_materials)
    ax.set_title('Total Absolute Impulse = ∫ |F(t)| dt\n(Magnitude of Momentum Transfer)', 
                 fontsize=16, fontweight='bold', fontfamily='DejaVu Sans')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'absolute_impulse_boxplot.png', dpi=300, bbox_inches='tight')
    ...
    # PLOT 3: Peak Force
    fig, ax = plt.subplots(1, 1, figsize=(10, 8))
    sns.boxplot(data=df, x='material_type', y='peak_force', ax=ax, order=existing_materials)
    ax.set_title('Peak Force Comparison', fontsize=16, fontweight='bold', fontfamily='DejaVu Sans')
    ...
    fig, ax = plt.subplots(1, 1, figsize=(10, 8))
    sns.boxplot(data=df, x='material_type', y='impact_duration_ms', ax=ax, order=existing_materials)
    ax.set_title('Impact Duration Comparison', fontsize=16, fontweight='bold', fontfamily='DejaVu Sans')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'impact_duration_boxplot.png', dpi=300, bbox_inches='tight')
    ...
    if len(df) > 1:
        ...
        for material, color in zip(materials, colors):
            ...
            ax.scatter(material_data['peak_force'], material_data['total_abs_impulse'], 
                      label=material, alpha=0.7, s=100, color=color)
        
        ax.set_xlabel('Peak Force (N)', fontsize=14, fontfamily='DejaVu Sans')
        ax.set_ylabel('Total Impulse Magnitude (N⋅s)', fontsize=14, fontfamily='DejaVu Sans')
        ax.set_title('Impulse vs Peak Force Relationship', fontsize=16, fontweight='bold', fontfamily='DejaVu Sans')
        ...
        plt.tight_layout()
        plt.savefig(output_dir / 'impulse_vs_force_scatter.png', dpi=300, bbox_inches='tight')
    ...
    if len(df) > 1:
        ...
        # Impulse ranking
        impulse_means = material_summary[('total_abs_impulse', 'mean')].sort_values(ascending=True)
        impulse_stds = material_summary[('total_abs_impulse', 'std')].reindex(impulse_means.index)
        
        y_pos = np.arange(len(impulse_means))
        bars1 = ax1.barh(y_pos, impulse_means.values, xerr=impulse_stds.values,
                        alpha=0.7, capsize=5, color='skyblue', edgecolor='navy')
        ax1.set_yticks(y_pos)
        ax1.set_yticklabels(impulse_means.index)
        ax1.set_xlabel('Total Impulse Magnitude (N⋅s)', fontsize=12, fontfamily='DejaVu Sans')
        ax1.set_title('Material Ranking by Impulse', fontsize=14, fontweight='bold', fontfamily='DejaVu Sans')
        ax1.grid(True, alpha=0.3, axis='x')
        
        # Add value labels
        for i, (mean, std) in enumerate(zip(impulse_means.values, impulse_stds.values)):
            ax1.text(mean + std * 1.1, i, f'{mean:.0f}±{std:.0f}', 
                    ha='left', va='center', fontweight='bold')
        
        # Force ranking
        force_means = material_summary[('peak_force', 'mean')].sort_values(ascending=True)
        force_stds = material_summary[('peak_force', 'std')].reindex(force_means.index)
        
        y_pos = np.arange(len(force_means))
        bars2 = ax2.barh(y_pos, force_means.values, xerr=force_stds.values,
                        alpha=0.7, capsize=5, color='lightcoral', edgecolor='darkred')
        ax2.set_yticks(y_pos)
        ax2.set_yticklabels(force_means.index)
        ax2.set_xlabel('Peak Force (N)', fontsize=12, fontfamily='DejaVu Sans')
        ax2.set_title('Material Ranking by Peak Force', fontsize=14, fontweight='bold', fontfamily='DejaVu Sans')
        ax2.grid(True, alpha=0.3, axis='x')
        
        # Format x-axis and add labels
        ax2.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x/1000:.0f}k'))
        for i, (mean, std) in enumerate(zip(force_means.values, force_stds.values)):
            ax2.text(mean + std * 1.1, i, f'{mean/1000:.0f}k±{std/1000:.0f}k', 
                    ha='left', va='center', fontweight='bold')
        
        plt.tight_layout()
        plt.savefig(output_dir / 'material_ranking_bars.png', dpi=300, bbox_inches='tight')
...
def create_si_boxplots_from_impulse_results(results: List[Dict], output_dir: Path):
    ...
    valid_results = [r for r in results if 'error' not in r and 
                    'total_abs_impulse' in r and 'peak_force' in r]
...
def create_latex_table(results: List[Dict], output_dir: Path):
    ...
    material_stats = df.groupby('material_type', observed=False).agg({
    ...
    stnd_impulse = material_stats.loc['STND', ('total_abs_impulse', 'mean')] if 'STND' in material_stats.index else 0
    stnd_energy = material_stats.loc['STND', ('equivalent_kinetic_energy', 'mean')] if 'STND' in material_stats.index else 0
...
\\hline\n"""
    ...
    for material in material_order:
        ...
        # Convert units
        impulse_kNs = material_stats.loc[material, ('total_abs_impulse', 'mean')] / 1000  # N⋅s to kN⋅s
        impulse_std_kNs = material_stats.loc[material, ('total_abs_impulse', 'std')] / 1000
        force_kN = material_stats.loc[material, ('peak_force', 'mean')] / 1000  # N to kN
        force_std_kN = material_stats.loc[material, ('peak_force', 'std')] / 1000
        energy_MJ = material_stats.loc[material, ('equivalent_kinetic_energy', 'mean')] / 1e6  # J to MJ
        ...
        if material == 'STND':
            ...
            impulse_pct_val = ((impulse_kNs * 1000 - stnd_impulse) / stnd_impulse * 100) if stnd_impulse > 0 else 0
            energy_pct_val = ((energy_MJ * 1e6 - stnd_energy) / stnd_energy * 100) if stnd_energy > 0 else 0
...
def create_impulse_report(results: List[Dict], output_dir: Path):
    ...
    # ANOVA test
    materials = df['material_type'].unique()
    impulse_groups = [df[df['material_type'] == mat]['total_impulse'].values 
    ...
    with open(output_dir / 'impulse_analysis_report.txt', 'w') as f:
        ...
        # Method ranking
        method_means = df.groupby('material_type')['total_abs_impulse'].mean().sort_values(ascending=False)
        ...
        for i, (method, mean_impulse) in enumerate(method_means.items(), 1):
            ...
            f.write(f"{i}. {method}: {mean_impulse:.6f} N⋅s magnitude "
    ...
    print(f"📄 Impulse analysis report saved to: {output_dir / 'impulse_analysis_report.txt'}")
...
def create_impulse_ranking_plot(df: pd.DataFrame, output_dir: Path):
    ...
    material_summary_sorted = material_summary.sort_values(('total_abs_impulse', 'mean'), ascending=True)
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    fig.suptitle('Fishing Line Method Performance Ranking (Impulse-Based)', 
                 fontsize=16, fontweight='bold', fontfamily='DejaVu Sans')
    ...
    for i, (impulse, error) in enumerate(zip(impulses, errors)):
        ...
        ax1.text(label_x, i, f'{impulse:+.4f}', 
                ha='left' if impulse >= 0 else 'right', va='center', fontweight='bold')
...
def create_method_comparison_plots(ke_results: List[Dict], impulse_results: List[Dict], 
                                 output_dir: Path):
    """
    ...
    # Filter valid results
    ke_valid = [r for r in ke_results if 'error' not in r and 
    ...
    fig.suptitle('Kinetic Energy vs Impulse Analysis Comparison', 
                 fontsize=16, fontweight='bold', fontfamily='DejaVu Sans')
    
    # 1. Method correlation
    axes[0,0].scatter(merged['kinetic_energy'], merged['equivalent_kinetic_energy'], 
                     alpha=0.6, s=50)
    ...
    if len(merged) > 2:
        corr = np.corrcoef(merged['kinetic_energy'], merged['equivalent_kinetic_energy'])[0,1]
        z = np.polyfit(merged['kinetic_energy'], merged['equivalent_kinetic_energy'], 1)
        p = np.poly1d(z)
        x_line = np.linspace(merged['kinetic_energy'].min(), merged['kinetic_energy'].max(), 100)
    ...
    axes[0,1].scatter(merged['initial_velocity'].abs(), merged['equivalent_velocity'], 
                     alpha=0.6, s=50)
    ...
    sns.boxplot(data=merged, x='material_type_impulse', y='total_abs_impulse', ax=axes[1,1])
    ...
    ke_ranking = merged.groupby('material_type_ke')['kinetic_energy'].mean().sort_values(ascending=False)
    impulse_ranking = merged.groupby('material_type_impulse')['total_abs_impulse'].mean().sort_values(ascending=False)
    
    # Create ranking comparison table
    axes[1,2].axis('off')
    
    ranking_data = []
    for i, (ke_mat, impulse_mat) in enumerate(zip(ke_ranking.index, impulse_ranking.index)):
        ranking_data.append([
            f"#{i+1}",
            ke_mat,
    ...
    table = axes[1,2].table(cellText=ranking_data,
                           colLabels=['Rank', 'KE Method', 'Energy (J)', 'Impulse Method', 'Impulse (N⋅s)'],
...
def test_unicode_support():
    ...
    try:
        ...
        ax.text(0.5, 0.5, "Test: N⋅s and N·s", fontsize=16, ha='center', va='center', fontfamily='DejaVu Sans')
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
