...
from .shared import DEFAULT_SAMPLING_RATE
from .shared import IMPACT_THRESHOLD_FACTOR
from .shared import LINE_MASS_FRACTION
from .shared import calculate_total_force
from .shared import extract_material_code
from .shared import extract_sample_number
...
class ImpulseAnalyzer:
    ...
    def find_impact_boundaries(self, force: np.ndarray) -> Tuple[int, int]:
        ...
        peak_region_mask = (threshold_indices >= peak_idx - 100) & (
            threshold_indices <= peak_idx + 100
        )
    ...
    def calculate_impulse_metrics(self, force: np.ndarray, time: np.ndarray) -> Dict:
        ...
        try:
            ...
            # Timing
            impact_duration = (
                time[impact_end] - time[impact_start]
            ...
            if self.total_mass and self.total_mass > 0:
                ...
                equivalent_kinetic_energy = (
                    0.5 * self.total_mass * equivalent_velocity**2
                )
            ...
            return {
                ...
                "impact_start_time": (
                    time[impact_start] if len(time) > impact_start else 0
                ),
    ...
    def plot_impulse_boundaries(
        ...
        if impact_end > impact_start:
            ...
            peak_idx = impact_start + np.argmax(
                np.abs(force[impact_start : impact_end + 1])
            )
            ax2.plot(
                time[peak_idx] * 1000,
                force[peak_idx],
    ...
    def analyze_csv_file(
        ...
        try:
            ...
            time_data, actual_sampling_rate = get_time_array(
                df, len(force_data), self.sampling_rate
            )
            ...
            if not self.material_code:
                ...
                self.mass_info = get_system_mass(
                    self.material_code, self.include_line_mass, self.line_mass_fraction
                )
                self.total_mass = self.mass_info["total_mass_kg"]
            ...
            if "error" not in result:
                ...
                if show_plot:
                    self.plot_impulse_boundaries(
                        force_data,
                        time_data,
                        result["impact_start_idx"],
                        result["impact_end_idx"],
                        csv_path.name,
                    )
...
def run_impulse_analysis(
    ...
    print("-" * 60)
    print(
        f"✅ ANALYSIS COMPLETE: {successful_analyses}/{len(csv_files)} files successful"
    )
    print()
        ...
        if valid_results:
            ...
            if "material_type" in results_df.columns:
                ...
                for material in sorted(results_df["material_type"].unique()):
                    ...
                    print(
                        f"   {material:4s}: {count:2d} samples | "
