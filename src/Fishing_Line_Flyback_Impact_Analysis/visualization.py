...
class ImpactVisualizer:
    ...
    def create_executive_summary_plot(
        ...
        materials = (
            sorted(self.df["material_type"].unique())
            if "material_type" in self.df.columns
            else []
        )
        energy_range = (
            self.df["kinetic_energy"].min(),
            self.df["kinetic_energy"].max(),
        )
        avg_overestimation = (
            self.df["overestimation_factor"].mean()
...
💪 Max Force: {self.df['max_force'].max():.0f} N
        ...
        if "material_type" in self.df.columns:
            ...
            for material, color in zip(materials, colors):
                ...
                ax2.scatter(
                    material_data["max_force"],
                    material_data["kinetic_energy"],
                    label=material,
                    alpha=0.7,
                    s=50,
                    color=color,
            ...
            ax2.scatter(
                self.df["max_force"], self.df["kinetic_energy"], alpha=0.6, s=50
            )
        ...
        if len(self.df) > 2:
            ...
            p = np.poly1d(z)
            ax2.plot(
                self.df["max_force"].sort_values(),
                p(self.df["max_force"].sort_values()),
                "r--",
                alpha=0.8,
                linewidth=2,
            )
    ...
    def create_methodology_comparison_plot(
        self, output_path: Optional[Path] = None
    ) -> plt.Figure:
        ...
        fig.suptitle(
            "Corrected Methodology: Before vs After Comparison",
            fontsize=16,
            fontweight="bold",
        )
        ...
        if all(
            ...
            for col in ["kinetic_energy_corrected", "kinetic_energy_overestimated"]
        ):
            energy_data = pd.melt(
                self.df,
                id_vars=["material_type"],
            ...
            sns.boxplot(
                data=energy_data, x="material_type", y="energy", hue="method", ax=ax1
            )
            ax1.set_yscale("log")
        ...
        if all(
            ...
            ax2.scatter(
                self.df["kinetic_energy_corrected"],
                self.df["kinetic_energy_overestimated"],
                alpha=0.6,
                s=50,
            )
            ...
            min_val = min(
                self.df["kinetic_energy_corrected"].min(),
                self.df["kinetic_energy_overestimated"].min(),
            )
            max_val = max(
                self.df["kinetic_energy_corrected"].max(),
        ...
        if all(col in self.df.columns for col in ["total_time", "decel_end_time"]):
            ax3.scatter(
                self.df["total_time"], self.df["decel_end_time"], alpha=0.6, s=50
            )
            ax3.plot(
                [0, self.df["total_time"].max()],
                [0, self.df["total_time"].max()],
                "r--",
                alpha=0.8,
                label="Full duration",
        ...
        if "overestimation_factor" in self.df.columns:
            ...
            bars = ax4.bar(
                improvement_stats.keys(),
                improvement_stats.values(),
    ...
    def create_material_analysis_plot(
        self, output_path: Optional[Path] = None
    ) -> plt.Figure:
        ...
        # Energy distribution
        sns.violinplot(
            data=self.df, x="material_type", y="kinetic_energy", ax=axes[0, 0]
        )
        ...
        if "initial_velocity" in self.df.columns:
            sns.boxplot(
                data=self.df, x="material_type", y="initial_velocity", ax=axes[0, 2]
            )
        ...
        if all(col in self.df.columns for col in ["decel_end_time", "total_time"]):
            self.df["decel_fraction"] = (
                self.df["decel_end_time"] / self.df["total_time"]
            )
            sns.boxplot(
                data=self.df, x="material_type", y="decel_fraction", ax=axes[1, 1]
            )
    ...
    def create_correlation_analysis_plot(
        ...
        if "material_type" in self.df.columns:
            ...
            ax2.scatter(
                self.df["max_force"], self.df["kinetic_energy"], alpha=0.6, s=60
            )
    ...
    def create_single_file_analysis_plot(
        ...
        fig.suptitle(
            f"Detailed Analysis: {analysis_result.get('filename', 'Unknown')}",
            fontsize=16,
        )
        ...
        ax1.fill_between(
            time_data[: decel_end_idx + 1],
            force_data[: decel_end_idx + 1],
            alpha=0.3,
            color="green",
            label="Deceleration Phase",
        )
        ax1.fill_between(
            time_data[decel_end_idx:],
            force_data[decel_end_idx:],
            alpha=0.3,
            color="orange",
            label="Rebound Phase",
        )
        ...
        ax2.axhline(
            cumulative_impulse[decel_end_idx], color="r", linestyle=":", alpha=0.7
        )
        ...
        # Energy comparison
        energies = [
            analysis_result.get("kinetic_energy_corrected", 0),
...
METHOD: Deceleration-only energy calculation
    ...
    def create_all_plots(self, output_dir: Union[str, Path]) -> None:
        ...
        try:
            ...
            fig1.savefig(
                output_path / "executive_summary.png", dpi=300, bbox_inches="tight"
            )
        ...
        try:
            ...
            fig2.savefig(
                output_path / "methodology_comparison.png", dpi=300, bbox_inches="tight"
            )
            plt.close(fig2)
        ...
        try:
            ...
            fig3.savefig(
                output_path / "material_analysis.png", dpi=300, bbox_inches="tight"
            )
            plt.close(fig3)
        ...
        try:
            ...
            fig4.savefig(
                output_path / "correlation_analysis.png", dpi=300, bbox_inches="tight"
            )
            plt.close(fig4)
...
Data Points: {len(force_data)}
    ...
    if show_analysis_preview:
        ...
        axes[1, 0].plot(
            time_data, force_data, "b-", linewidth=1, alpha=0.7, label="Force"
        )
        axes[1, 0].axvline(
            time_data[decel_end_idx],
            color="red",
...
Decel Impulse: {J_decel:.6f} N⋅s
        ...
        if window_size > 1:
            moving_avg = np.convolve(
                force_data, np.ones(window_size) / window_size, mode="same"
            )
            axes[1, 0].plot(
                time_data, force_data, "b-", alpha=0.5, linewidth=0.5, label="Raw"
            )
            axes[1, 0].plot(
                time_data,
                moving_avg,
                "r-",
                linewidth=2,
                label=f"Moving avg ({window_size} pts)",
            )
            axes[1, 0].set_xlabel("Time (s)")
            axes[1, 0].set_ylabel("Force (N)")
        ...
        if len(force_data) > 256:
            try:
                ...
                freqs, psd = scipy_signal.welch(
                    force_data, fs=1 / dt, nperseg=min(256, len(force_data) // 4)
                )
...
def quick_energy_check_plot(
    ...
    valid_results = [
        r
        for r in results
        if "kinetic_energy" in r
        and not pd.isna(r["kinetic_energy"])
        and r["kinetic_energy"] > 0
    ]
    ...
    ax1.axvline(
        1e-1, color="orange", linestyle="--", alpha=0.7, label="100 mJ (moderate)"
    )
...
def create_material_summary_plots(
...
def create_individual_analysis_plot(
    force_data: np.ndarray,
    time_data: np.ndarray,
    analysis_result: Dict,
    ...
    visualizer = ImpactVisualizer()
    fig = visualizer.create_single_file_analysis_plot(
        force_data, time_data, analysis_result, Path(output_path)
    )
    plt.close(fig)
...
> 10 J (high): {sum(1 for e in energies if e >= 10)} files
    ...
    if valid_results and "material_type" in valid_results[0]:
        df = pd.DataFrame(valid_results)
        material_stats = (
            df.groupby("material_type")["kinetic_energy"]
            .agg(["count", "mean"])
            .round(6)
        )
        ...
        for material, stats in material_stats.iterrows():
            summary += (
                f"{material}: {stats['count']} samples, avg {stats['mean']:.6f} J\n"
            )
...
if __name__ == "__main__":
    ...
    print(
        "This module provides comprehensive plotting capabilities for analysis results."
    )
    print("\nMain functions:")
...
def plot_impulse_force_box_si(
    results: pd.DataFrame, output_dir: Path, units: str = "SI"
):
    ...
    assert units in (
        ...
        "kSI",
        "mixed",
    ), "Units must be one of 'SI', 'kSI', or 'mixed'"
    unit_labels = {
    ...
    for ext in [".svg", ".png"]:
        fig.savefig(
            output_dir / f"plot-box-J-F-{units}{ext}", dpi=600, bbox_inches="tight"
        )
    plt.close(fig)
